...
// Добавляем пустые статы к начальным данным команд для локального состояния
const initialTeamsWithStats = initialTeamsData.map(team => ({
    ...team,
...
function App() {
  // --- Состояния ---
  const [teams, setTeams] = useState(initialTeamsWithStats); // Базовая инфо + вычисл. статы
  const [matches, setMatches] = useState([]); // Получаем из Firestore
  const [tournamentSettings, setTournamentSettings] = useState(defaultTournamentSettings); // Получаем из Firestore
  const [isLoading, setIsLoading] = useState(true); // Флаг загрузки

  // UI состояния
  const [view, setView] = useState('matches');
  const [selectedMatch, setSelectedMatch] = useState(null);
  const [language, setLanguage] = useState(() => localStorage.getItem('language') || 'cs'); // Язык оставим в localStorage
  ...
  const updatePlayoffDisplay = useCallback(() => {
    ...
    ['A', 'B', 'C'].forEach(group => {
     ...
     let updatedMatchesForDisplay = matches.map(match => {
         if (match.round !== 'group') {
             ...
             // Обновляем статус для UI только если он был 'waiting'
             if (displayMatch.team1 && displayMatch.team2 && displayMatch.status === 'waiting') {
                 displayMatch.status = 'not_started';
             } else if ((!displayMatch.team1 || !displayMatch.team2) && displayMatch.status !== 'waiting') {
                  // Если команды пропали, но статус уже был 'not_started' или 'in_progress', не меняем его обратно на 'waiting'
                  // Оставляем как есть, чтобы сохранить введенные очки, если они были
             }
             return displayMatch;
         }
         return match;
    ...
    // Сравниваем старый и новый массив матчей, чтобы избежать лишних ререндеров
    if (JSON.stringify(matches) !== JSON.stringify(updatedMatchesForDisplay)) {
        setMatches(updatedMatchesForDisplay); // Обновляем состояние матчей для UI
        console.log("Playoff display updated in state");
    } else {
         console.log("Playoff display unchanged");
    }
  ...
  const updateMatchScore = useCallback(async (matchId, set, team, scoreStr) => {
    ...
    let newWinner = tempUpdatedMatch.winner;
    let newStatus = tempUpdatedMatch.status;

    const { set1Team1, set1Team2, set2Team1, set2Team2, set3Team1, set3Team2 } = tempUpdatedMatch;

    // Логика определения статуса и победителя (остается сложной, но важной)
    if (tempUpdatedMatch.round === 'final') {
         let team1Wins = (set1Team1 > set1Team2 ? 1 : 0) + (set2Team1 > set2Team2 ? 1 : 0) + (set3Team1 > set3Team2 ? 1 : 0);
         let team2Wins = (set1Team2 > set1Team1 ? 1 : 0) + (set2Team2 > set2Team1 ? 1 : 0) + (set3Team2 > set3Team1 ? 1 : 0);
         if (team1Wins >= 2) { newWinner = tempUpdatedMatch.team1; newStatus = 'completed'; }
         else if (team2Wins >= 2) { newWinner = tempUpdatedMatch.team2; newStatus = 'completed'; }
         else if ((set1Team1 > 0 || set1Team2 > 0) || (set2Team1 > 0 || set2Team2 > 0) || (set3Team1 > 0 || set3Team2 > 0)) { newWinner = null; newStatus = 'in_progress';} // Если есть очки, но не завершен
         else {newWinner = null; newStatus = 'not_started';} // Если все по нулям
    } else if (tempUpdatedMatch.status === 'tie_needs_tiebreak' && set === 3) {
         // Тайбрейк не-финала
         if (set3Team1 >= 5 && set3Team1 > set3Team2 && set3Team1 >= set3Team2 + 1) { newWinner = tempUpdatedMatch.team1; newStatus = 'completed'; } // Учтем разницу в 1 очко минимум для >5
         else if (set3Team2 >= 5 && set3Team2 > set3Team1 && set3Team2 >= set3Team1 + 1) { newWinner = tempUpdatedMatch.team2; newStatus = 'completed'; }
         else { newWinner = null; newStatus = 'tie_needs_tiebreak'; } // Продолжаем или неверный ввод
    } else if (set >= 2 && tempUpdatedMatch.round !== 'final') {
        // Проверка после 2го или 3го сета (если был тайбрейк) для не-финала
         ...
         if (team1Sets === 2) { newWinner = tempUpdatedMatch.team1; newStatus = 'completed'; }
         else if (team2Sets === 2) { newWinner = tempUpdatedMatch.team2; newStatus = 'completed'; }
         else if (team1Sets === 1 && team2Sets === 1) {
             // Матч 1:1 - проверяем правило
             if (useTotalPointsRule) {
                 const team1Total = set1Team1 + set2Team1; const team2Total = set1Team2 + set2Team2;
                 if (team1Total > team2Total) { newWinner = tempUpdatedMatch.team1; newStatus = 'completed_by_points'; }
                 else if (team2Total > team1Total) { newWinner = tempUpdatedMatch.team2; newStatus = 'completed_by_points'; }
                 else { newWinner = null; newStatus = 'tie_needs_tiebreak'; } // Равенство очков -> тайбрейк
             } else {
                 newWinner = null; newStatus = 'tie_needs_tiebreak'; // Всегда тайбрейк
             }
         } else if ((set1Team1 > 0 || set1Team2 > 0) || (set2Team1 > 0 || set2Team2 > 0)) {
             // Счет 1:0 или 0:1 после 2го сета
             newWinner = null; newStatus = 'in_progress';
         } else {
             // Все по нулям после 2го сета
             newWinner = null; newStatus = 'not_started';
         }
    } else if (set === 1 && tempUpdatedMatch.round !== 'final') {
        // После первого сета
         newWinner = null;
         newStatus = (set1Team1 > 0 || set1Team2 > 0) ? 'in_progress' : 'not_started';
         // Если сбросили первый сет на 0:0, а второй уже был сыгран, надо перепроверить статус
    ...
    try {
        ...
        console.log(`Match ${matchId} updated in Firestore`);
    } catch (error) {
  ...
  const changeLanguage = (lang) => {
    if (translations[lang]) {
        ...
        console.warn(`Language "${lang}" not found. Falling back to 'cs'.`);
        setLanguage('cs');
  ...
  // --- Функции рендеринга ---
  // ВАЖНО: Используют состояния `matches`, `teams`, `tournamentSettings`,
  // которые обновляются через Firestore `onSnapshot`.
  const renderMatches = useCallback(() => {
     // ... (Код рендеринга матчей, как в предыдущем ответе)
       return (
          ...
          {!isLoading && matches.length > 0 && (
             <div className="bg-gradient-to-r from-[#C1CBA7] to-[#0B8E8D]/10 p-4 md:p-6 rounded-xl shadow-lg">
               <table className="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
                 ...
                 <tbody>
                   {[...matches] // Сортируем копию для отображения
                       .sort((a, b) => {
                       ...
                       .map(match => {
                        ...
                        else { statusIcon = <FaRegClock className="mr-1 text-gray-500" />; statusClass = 'text-gray-600'; } // not_started и др.
                        ...
                        return (
                            <tr key={match.id} className={`border-b transition-colors duration-150 ease-in-out ${canOpenDetail ? 'hover:bg-[#0B8E8D]/10 cursor-pointer' : 'opacity-70'}`}
                                onClick={canOpenDetail ? () => { setView('matchDetail'); setSelectedMatch(match); } : undefined}>
       ...
       return (
        <div className="p-4">
          <h2 className="text-2xl font-bold mb-6 text-indigo-700 flex items-center">
           ...
           {isLoading && <p className="text-center p-4">{t.loading || 'Загрузка...'}</p>}
           {!isLoading && teams.length === 0 && <p className="text-center p-4">{t.noTeams || 'Команды не найдены.'}</p>}
               ...
               {['A', 'B', 'C'].map(group => {
                 ...
                 return (
                   <div key={group} className={`bg-gradient-to-r ${colors.lightBg} rounded-xl shadow-lg overflow-hidden`}>
                     ...
                     <div className="p-4 overflow-x-auto">
                        {groupTeams.length === 0 ? <p className="text-center text-gray-500 py-4">{t.noTeamsInGroup || 'Нет команд в этой группе'}</p> : (
                           <table className="min-w-full bg-white shadow-sm rounded-lg overflow-hidden">
                             <thead className="bg-gray-50">
                               <tr>
                                 ...
                                 <th className="p-3 text-center text-xs md:text-sm font-semibold text-gray-700" title={t.winsLosses || 'Победы / Поражения'}>{t.winsLossesShort}</th>
                                 <th className="p-3 text-center text-xs md:text-sm font-semibold text-gray-700" title={t.sets || 'Сеты Выиграно / Проиграно'}>{t.setsShort}</th>
                                 <th className="p-3 text-center text-xs md:text-sm font-semibold text-gray-700" title={t.setsDifference || 'Разница сетов'}>{t.setsDiffShort}</th>
                               </tr>
                             </thead>
                             <tbody>
                               {[...groupTeams] // Сортируем копию
                                 .sort((a, b) => b.points - a.points || (b.setsWon - b.setsLost) - (a.setsWon - a.setsLost) || b.setsWon - a.setsWon || a.name.localeCompare(b.name))
                                 .map((team, index) => (
           ...
           <div className="bg-white rounded-xl shadow-2xl p-0 w-full max-w-lg overflow-hidden">
             ...
             <div className="bg-gradient-to-r from-[#0B8E8D] to-[#06324F] p-6 text-white">
               <div className="flex justify-between items-center">
                 ...
                 <button onClick={() => { setView('matches'); setSelectedMatch(null); }} className="text-white hover:text-red-200 transition-colors duration-150">✕</button>
               </div>
             </div>
             {/* Содержимое */}
             <div className="p-6 max-h-[80vh] overflow-y-auto"> {/* Добавлен max-height и overflow */}
               <div className="flex flex-wrap justify-between items-center mb-6 gap-y-2">
                 ...
                 <div className="text-gray-600 flex items-center"><FaCalendarAlt className="mr-2 text-indigo-500" />{currentMatchData.time} ({t.court} {currentMatchData.court})</div>
               </div>
               ...
               <div className="mt-6 flex justify-end">
                 <button onClick={() => { setView('matches'); setSelectedMatch(null); }} className="w-full bg-gradient-to-r from-[#0B8E8D] to-[#06324F] text-white py-2 px-4 rounded-lg hover:from-[#0B8E8D]/90 hover:to-[#06324F]/90 transition-all duration-200 shadow-md flex items-center justify-center"><FaCheck className="mr-2"/> {t.close || 'Закрыть'}</button>
               </div>
             </div>
           </div>
         </div>
  ...
  const renderRulesModal = useCallback(() => {
    // ... (Код рендеринга модалки правил, как в предыдущем ответе)
     return (
        <div className="fixed inset-0 bg-gray-900 bg-opacity-80 flex items-start justify-center p-4 z-[60] backdrop-blur-sm overflow-y-auto"> {/* Увеличен z-index */}
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center space-x-2">
                   {Object.keys(languageNames).filter(lang => translations[lang]).map(lang => (
                        <button key={lang} onClick={() => changeLanguage(lang)} className={`px-2 py-1 text-xs rounded transition-colors duration-150 ${language === lang ? 'bg-white text-[#0B8E8D] shadow-sm' : 'bg-[#0B8E8D]/80 text-white hover:bg-[#0B8E8D]/90'}`} title={languageNames[lang]}>{lang.toUpperCase()}</button>
                    ))}
            ...
            <div className="p-6 max-h-[70vh] overflow-y-auto">
              ...
              <div className="bg-gray-50 p-4 rounded-lg shadow-sm mb-6 border border-gray-200">
                ...
                <div className="flex items-center space-x-3">
                  <input
                    type="checkbox" id="useTotalPointsForTie"
                    checked={tournamentSettings.useTotalPointsForTie}
                    onChange={(e) => handleSettingsChange(e.target.checked)} // Передаем boolean значение
                 ...
                 <p className="text-xs text-gray-500 mt-2">
                     {tournamentSettings.useTotalPointsForTie
                      ? t.tieRules?.usePointsOptionDescription || "При 1:1 выигрыш по очкам, при равенстве - тайбрейк."
                      : t.tieRules?.useTiebreakOptionDescription || "При 1:1 всегда играется тайбрейк."}
                 </p>
              </div>
              {/* Описание правил */}
  ...
  return (
    <>
      <div className="min-h-screen flex flex-col md:flex-row bg-gray-50">
        ...
        <aside className="w-full md:w-64 bg-white shadow-md md:h-screen md:sticky top-0 z-20 border-t border-gray-200"> {/* Уменьшен z-index */}
           <div className="p-4 border-b border-gray-200 flex justify-between items-center">
             ...
             <div className="flex space-x-1">
                {Object.keys(languageNames).filter(lang => translations[lang]).map(lang => (
                     <button key={lang} onClick={() => changeLanguage(lang)} className={`px-2 py-1 text-xs rounded transition-colors duration-150 ${language === lang ? 'bg-[#0B8E8D] text-white shadow-sm' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`} title={languageNames[lang]}>{lang.toUpperCase()}</button>
                 ))}
             </div>
           </div>
        ...
        <nav className="fixed bottom-0 left-0 right-0 bg-white shadow-lg p-2 flex justify-around md:hidden z-30 border-t border-gray-200"> {/* Уменьшен z-index */}
      ...
      {/* Модальное окно деталей матча (рендер выше) */}
